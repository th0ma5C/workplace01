...
const move = function (step) {
    ...
    container.style.transform = `translateX(${distanceBetweenContent * (position - Math.ceil(contentToShow))
        }px`;
...
const init = () => {
    ...
    insertClonedSlider();
    // 最初的移動
    container.style.transform = `translateX(${-distanceBetweenContent * Math.ceil(contentToShow)
        }px)`;
};
init();
